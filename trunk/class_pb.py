...
class phonebook:
	def __init__(s):
		...
		try: import appuifw2
		except: s.appuifw2disponibile = 0
		else:
		...
		if s.appuifw2disponibile: s.oldnavi_text = s.appuifw2.app.navi_text
	def ___check(s,x):
		try: x[0].encode('latin').replace(' ','').decode('hex')
		except: return 1
		else: return 0
	def ___singolo(s):
		s.appuifw.app.title = __title__
		if s.appuifw2disponibile: s.appuifw2.app.navi_text = u'%s' % __version__
		s.db_rubr=s.contacts.ContactsDb()
		s.nomi_r=[]
		s.nomi_posizione=[]
		for i in s.db_rubr: s.nomi_posizione.append((s.db_rubr[i].title,i))
		...
		if s.i!=None:
			...
			for linea in s.db_rubr[s.i_contatto].as_vcard().splitlines():
				if linea.find('TEL;')==0 and linea.find(':')!=-1: s.num_trovati.append(linea.split(':')[1].decode('latin'))
			if len(s.num_trovati)==0: return None 
			elif len(s.num_trovati)==1: s.destinatario=s.num_trovati[0]
			else:
				s.indice=s.appuifw.popup_menu(s.num_trovati,s.nomi_r[s.i])
				if s.indice!=None: s.destinatario=s.num_trovati[s.indice]
				else: return None
		...
		if len(s.i):
			...
			for i_scelto in s.i:
				...
				for linea in s.db_rubr[s.i_contatto].as_vcard().splitlines():
					if linea.find('TEL;')==0 and linea.find(':')!=-1: s.num_trovati.append(linea.split(':')[1].decode('latin'))
				if len(s.num_trovati)==0: return None
				elif len(s.num_trovati)==1: s.lista_finale.append((s.nomi_r[i_scelto],s.num_trovati[0]))
				else: 
					s.indice=s.appuifw.popup_menu(s.num_trovati,s.nomi_r[i_scelto])
					if s.indice!=None: s.lista_finale.append((s.nomi_r[i_scelto],s.num_trovati[s.indice]))
			if s.lista_finale: return s.lista_finale
			else: return None
		else: return None
	def ___giveback(s):
		...
		try: s.appuifw.app.orientation = s.oldorientation
		except: pass
		if s.appuifw2disponibile: s.appuifw2.app.navi_text = s.oldnavi_text
	def get(s, multi=0):
		if multi == 1: s.data = s.___multiplo()
		else: s.data = s.___singolo()
		s.___giveback()
		return s.data
